...
if(estimator %in% c("did", "all")) {
	message("Estimating using Callaway and Sant'Anna (2021)")
	...
	if(is.null(tidy_did)) warning("Callaway and Sant'Anna (2021) Failed")
}

# sunab ------------------------------------------------------------------------
if(estimator %in% c("sunab", "all")) {
	message("Estimating using Sun and Abraham (2021)")
	...
	if(is.null(tidy_sunab)) warning("Sun and Abraham (2021) Failed")
}
...
# Bind results together --------------------------------------------------------
	...
	out = data.table::rbindlist(list(
		...
		"Callaway and Sant'Anna (2021)" = tidy_did,
		"Sun and Abraham (2021)" = tidy_sunab,
...
plot_event_study = function(out, separate = TRUE, horizon = NULL) {
	...
	# Completing
	out <- out %>% 
	   complete(estimator, term, fill = list(estimate = 0, std.error = 0))
	...
	# Subset color scales
	color_scale = c("TWFE" = "#374E55", 
	                "Gardner (2021)" = "#DF8F44", 
	                "Callaway and Sant'Anna (2021)" = "#00A1D5", 
	                "Sun and Abraham (2021)" = "#B24745", 
	...
	ggplot2::ggplot(
      ...
      mapping = ggplot2::aes(
		...
		ggplot2::labs(y = "Point Estimate and 95% Confidence Intervals", 
	   ...
	   scale_x_continuous(breaks = seq(horizon[1], horizon[2])) + 
